...
**SolveOrder** | **int** | &lt;p&gt;Priority in which the dimension or member is calculated (0 to 127). Higher solve order means the member is calculated later. Members did not assign a solve order inherit it from their dimension.&lt;/p&gt; | [optional] 
