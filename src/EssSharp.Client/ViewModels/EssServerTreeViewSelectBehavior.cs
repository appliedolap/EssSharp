...
{
    public class EssServerTreeViewSelectBehavior : Behavior<CheckBox>
    {
        ...
        protected virtual void ApplyHierarchicalSelection( CheckBox checkBox )
        {
            ...
            foreach ( TreeViewNodeHolderView child in holder.NodeChildren.Where(x => x is TreeViewNodeHolderView) )
            {
                ...
                if ( child.NodeView is IStackLayout layout )
                    childCheckBox = layout.FirstOrDefault(v => v is CheckBox) as CheckBox;
                else
                    childCheckBox = holder.NodeView as CheckBox;
        ...
        protected virtual void CheckStateItself( TreeViewNodeHolderView holder, bool forcedSemiSelected = false )
        {
            ...
            if ( holder.NodeView is IStackLayout layout )
                mainCheckBox = layout.FirstOrDefault(v => v is CheckBox) as CheckBox;
            else
                mainCheckBox = holder.NodeView as CheckBox;
